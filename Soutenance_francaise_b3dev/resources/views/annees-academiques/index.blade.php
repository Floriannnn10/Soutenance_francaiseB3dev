        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-6 border-b border-gray-200 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-medium text-gray-900">Liste des Années Académiques</h3>
                        <p class="mt-1 text-sm text-gray-600">Gérez les années académiques de votre établissement</p>
                    </div>
                ...
                <div class="p-6 text-gray-900">
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                                @if($annee->statut === 'En cours') bg-green-100 text-green-800
                                                @elseif($annee->statut === 'À venir') bg-yellow-100 text-yellow-800
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                                @if($annee->actif) bg-green-100 text-green-800
                                                @else bg-gray-100 text-gray-800
                                                @endif">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {{ $annee->semestres->count() }} semestre(s)
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex flex-col space-y-1">
                                                ...
                                                @if($annee->actif)
                                                    <form action="{{ route('annees-academiques.deactivate', $annee) }}"
                                                          method="POST" class="inline">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit"
                                                                class="text-orange-600 hover:text-orange-900 flex items-center" title="Désactiver"
                                                                onclick="return confirm('Êtes-vous sûr de vouloir désactiver cette année académique ?')">
                                                            <i class="fas fa-pause mr-2"></i>Désactiver
                                                        </button>
                                                    </form>
                                                @else
                                                    <form action="{{ route('annees-academiques.activate', $annee) }}"
                                                          method="POST" class="inline">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit"
                                                                class="text-green-600 hover:text-green-900 flex items-center" title="Activer"
                                                                onclick="return confirm('Êtes-vous sûr de vouloir activer cette année académique ?')">
                                                            <i class="fas fa-play mr-2"></i>Activer
                                                        </button>
                                                    </form>
                                                @endif
                                                <form action="{{ route('annees-academiques.destroy', $annee) }}"
                                                      method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                    ...
                    <div class="mt-6 flex items-center justify-between">
                        <div class="text-sm text-gray-700">
                            Affichage de <span class="font-medium">{{ $anneesAcademiques->firstItem() ?? 0 }}</span>
                            à <span class="font-medium">{{ $anneesAcademiques->lastItem() ?? 0 }}</span>
                            sur <span class="font-medium">{{ $anneesAcademiques->total() }}</span> résultats
                        </div>
                        <div>
