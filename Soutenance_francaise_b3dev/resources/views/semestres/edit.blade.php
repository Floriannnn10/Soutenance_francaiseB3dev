   ...
   <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Modifier le Semestre') }}: {{ $semestre->nom }}
            </h2>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('semestres.update', $semestre) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <x-text-input id="nom" name="nom" type="text" class="mt-1 block w-full" 
                                          :value="old('nom', $semestre->nom)" required autofocus 
                            ...
                            <select id="annee_academique_id" name="annee_academique_id" 
                                ...
                                @foreach($anneesAcademiques as $annee)
                                    <option value="{{ $annee->id }}" 
                                        {{ old('annee_academique_id', $semestre->annee_academique_id) == $annee->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <x-text-input id="date_debut" name="date_debut" type="date" class="mt-1 block w-full" 
                                          :value="old('date_debut', $semestre->date_debut?->format('Y-m-d'))" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="date_fin" name="date_fin" type="date" class="mt-1 block w-full" 
                                          :value="old('date_fin', $semestre->date_fin?->format('Y-m-d'))" required />
                        ...
                        <div class="flex items-center">
                            <input id="actif" name="actif" type="checkbox" value="1" 
                                   ...
                                   {{ old('actif', $semestre->actif) ? 'checked' : '' }}>
